...
class TextModelEvaluator:
    def __init__(self, text_columns, label_column="emotion", max_features=4500, ngram_range=(1, 2), static_state=42, vectorizer_type="tfidf"):
        ...
        self.vectorizer_class = TfidfVectorizer if vectorizer_type == "tfidf" else CountVectorizer
        self.base_models = {
            'Logistic Regression': LogisticRegression(max_iter=1000, class_weight='balanced', random_state=self.rand_state),
            'SVM': LinearSVC(class_weight='balanced', random_state=self.rand_state),
    ...
    def compare_models(self, train_df, cross_validate=False):
        for variant_name, column in self.text_columns.items():
            ...
            vectorizer = self.vectorizer_class(**self.vectorizer_settings)
            X_tfidf = vectorizer.fit_transform(X)

            for model_name, model in self.base_models.items():
                if cross_validate:
#                    cv = StratifiedKFold(n_splits=5, shuffle=True)  # no fixed random_state
                    scores = cross_val_score(model, X_tfidf, y, cv=5, scoring='accuracy')
                    mean_score = float(f"{scores.mean():.6f}")
                else:
                    model.fit(X_tfidf, y)
                    y_pred = model.predict(X_tfidf)
                    mean_score = float(f"{accuracy_score(y, y_pred):.6f}")
        ...
        self.best_vectorizer = self.vectorizer_class(**self.vectorizer_settings)
